...
def write_jsonl(path: Path, issues: list[dict]) -> None:
...
def merge_estimates(base: list, ours: list, theirs: list) -> list | None:
...
def merge_deps(base: list, ours: list, theirs: list) -> list | None:
...
def apply_specificity(refs: list) -> list:
...
def merge_references(base: list, ours: list, theirs: list) -> list | None:
    """Merge reference arrays with specificity subsumption."""
    def ref_to_tuple(r: dict) -> tuple:
...
def merge_issue(base: dict, ours: dict, theirs: dict) -> dict | None:
    ...
    for key in all_keys:
        ...
        if key in ARRAY_FIELDS:
            ...
            b = base_val if base_val is not None else []
            o = ours_val if ours_val is not None else []
            t = theirs_val if theirs_val is not None else []
