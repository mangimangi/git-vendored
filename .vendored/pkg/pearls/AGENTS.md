...
| `.claude/hooks/configure.sh` | **NO** | `install.hooks` | `templates/hooks/configure.sh` |
...
**Always use `prl` commands** — the `configure.sh` hook puts `prl` on PATH at session start. Never use `python3 prl.py` or `python3 .pearls/prl.py` directly.
...
**All new code in `prl.py` and `merge-driver.py` MUST include type annotations.** The codebase uses mypy in strict mode — CI will reject code without proper types.

- Use `Issue`, `Config`, and other TypedDicts defined in `prl.py` instead of `dict[str, Any]`
- Use `cast()` at JSON boundaries (e.g., `json.loads()`, `json.load()`)
- In `merge-driver.py`, import types under `TYPE_CHECKING` to avoid runtime dependency on `prl.py`
- Run `mypy prl.py merge-driver.py` locally before committing
...
Flags: `-e` estimator model, `-m` implementer model, `-i`/`-o` estimated tokens for implementation, `--ei`/`--eo` actual tokens consumed by the estimator. Use `--no-cost` to skip estimator cost tracking.
...
prl start proj-c3d2 --agent claude-opus-4-6
# ... fix the bug ...
