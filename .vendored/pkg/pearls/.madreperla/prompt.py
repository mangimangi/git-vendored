...
DEFAULT_REFINE_BODY = """I'd like to refine some features together and create straightforward pearls issues that are small in scope with clear acceptance criteria...if applicable we should specify requirements for extending docs and tests in addition to implementing the functionality of the feature. If anything is unclear or ambiguous let's dig into it together and get to a shared place of clarity. as part of this refinement - we should discern what epic the issue(s) will be created in (or if a new epic will be created).
...
DEFAULT_ESTIMATE_BODY = """then let's estimate some issues and add your token estimate using `prl estimate`...you can assume the issues will be implemented by {implementer} unless otherwise specified. we can disregard any existing estimates as they are likely stale!
...
DEFAULT_ONESHOT_BODY = """I'd like to refine some features together and create straightforward pearls issues that are small in scope with clear acceptance criteria...if applicable we should specify requirements for extending docs and tests in addition to implementing the functionality of the feature. If anything is unclear or ambiguous let's dig into it together and get to a shared place of clarity. as part of this refinement - we should discern what epic the issue(s) will be created in (or if a new epic will be created).
...
{eval_dimensions}
3. record the evaluation with `{eval_cli_example}` — prl eval auto-closes issues when all scores meet their threshold
...
def _get_epic_slugs(config: dict[str, Any]) -> list[str]:
...
def build_prompt_vars(config: dict[str, Any]) -> dict[str, str]:
    ...
    Auto-seeds built-in vars:
        ...
        {keep_epics} — computed from epics + prefix
        {prefix} — from prefix
    ...
    epic_slugs = _get_epic_slugs(config)
