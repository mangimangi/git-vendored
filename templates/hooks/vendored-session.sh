...
# Framework-owned: installed by git-vendored, do not edit manually.
set -euo pipefail
...
# ── Vendor feedback breadcrumb ────────────────────────────────────────────────
# On session start, emit a one-liner listing installed vendors with feedback info.
if [ -n "$VENDORS" ] && [ "$MODE" = "start" ]; then
    vendor_list=$(echo "$VENDORS" | tr '\n' ', ' | sed 's/, $//')
    if [ -n "$vendor_list" ]; then
        echo "Installed tools with feedback loops: ${vendor_list}. Run \`.vendored/feedback <tool>\` for details."
    fi
fi
